      ...
      <Container>
        <div className="flex items-center justify-between gap-3 rounded-full border border-border/40 bg-background/70 px-4 py-2.5 shadow-2xl shadow-primary/5 backdrop-blur-xl sm:px-5 lg:gap-6 lg:px-6">
                      ...
                      <div className="grid w-[620px] grid-cols-2 gap-2 p-3">
                        {services.map((service) => (
                          <div key={service.href}>
                            <Link
                              href={service.href}
                              className="block rounded-xl p-3 hover:bg-accent"
                            >
          ...
          <div className="flex items-center gap-2 lg:hidden">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background text-foreground transition-colors outline-none hover:bg-muted">
              ...
              <DropdownMenuContent align="end" className="mt-2 w-64">
                ...
                {services.map((service) => (
                  <DropdownMenuItem key={service.href} className="p-0">
                    <Link
                      href={service.href}
                      className="block w-full cursor-pointer px-3 py-2 text-muted-foreground"
                    >
                      {service.title}
                    </Link>
                  </DropdownMenuItem>
